...
class Apartment(Base):
    ...
    # ✅ New fields for short-stay tracking
    ...
    verified_by_agent = relationship("ListingAgent", back_populates="verified_apartments")
    project = relationship("Project", back_populates="apartments")
    
    external_calendars = relationship("ExternalCalendar", 
        primaryjoin="and_(Apartment.id==foreign(ExternalCalendar.property_id), ExternalCalendar.property_type=='apartments')")

    blocked_dates = relationship("BlockedDate",
        primaryjoin="and_(Apartment.id==foreign(BlockedDate.property_id), BlockedDate.property_type=='apartments')")

    sync_logs = relationship("SyncLog",
        primaryjoin="and_(Apartment.id==foreign(SyncLog.property_id), SyncLog.property_type=='apartments')")
    ...
    def get_by_status_for_user(cls, session, user_id, status):
        query = session.query(cls).filter(
            ...
            cls.is_approved == "approved",
            cls.display == 1,
            cls.availability_status.ilike(status)  
        )
    ...
    def as_dict(self, session=None):
        ...
        if session:
            # Media
            data['images'] = get_images(session, self.user_id, self.id, 'apartments')
            data['documents'] = get_documents(session, self.user_id, self.id, 'apartments')
            data['videos'] = get_videos(session, self.user_id, self.id, 'apartments')
            ...
            # Reviews
            data['reviews'] = [r.as_dict() for r in get_reviews(session, self.id, 'apartments')]
            data['average_rating'] = get_average_rating(session, self.id, 'apartments')
