...
class Commercial(Base):
    ...
    commercial_type = Column(String(100), nullable=False)
    ...
    is_approved = Column(String(20), default="pending")  # 'pending', 'approved', etc.
    ...
    user = relationship("User", back_populates="commercials")
    verified_by_agent = relationship("ListingAgent", back_populates="verified_commercials")
    project = relationship("Project", back_populates="commercial")
    
    external_calendars = relationship("ExternalCalendar", 
        primaryjoin="and_(Commercial.id==foreign(ExternalCalendar.property_id), ExternalCalendar.property_type=='commercial')")

    blocked_dates = relationship("BlockedDate",
        primaryjoin="and_(Commercial.id==foreign(BlockedDate.property_id), BlockedDate.property_type=='commercial')")

    sync_logs = relationship("SyncLog",
        primaryjoin="and_(Commercial.id==foreign(SyncLog.property_id), SyncLog.property_type=='commercial')")
    ...
    def as_dict(self, session=None):
        ...
        # Add media assets
        if session:
            data['images'] = get_images(session, self.user_id, self.id, 'commercial')
            data['documents'] = get_documents(session, self.user_id, self.id, 'commercial')
            data['videos'] = get_videos(session, self.user_id, self.id, 'commercial')
            cover_url = get_cover_image(session, self.user_id, self.id, 'commercial')
            ...
            # Add reviews & rating
            data['reviews'] = [r.as_dict() for r in get_reviews(session, self.id, 'commercial')]
            data['average_rating'] = get_average_rating(session, self.id, 'commercial')

            data['likes'] = get_likes(session, self.id)

            price = float(self.price)  
            if self.purpose == "Short Stay":
                data['Listed_price'] = f"KSh {price:,.2f} per day"  
