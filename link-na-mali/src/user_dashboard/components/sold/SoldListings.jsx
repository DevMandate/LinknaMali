...
const SoldListings = () => {
  ...
  const fetchSoldListings = useCallback(async () => {
    if (!userData?.user_id) {
    ...
    try {
      const response = await axios.get(
        `https://api.linknamali.ke/property/get-user-listings-by-status/${userData.user_id}?status=sold`
      );
      ...
      setError("Failed to fetch sold listings.");
    } finally {
      setLoading(false);
    }
  }, [userData?.user_id]);
  ...
  return (
    <div className="max-w-full mx-auto mt-8 p-4">
      <ToastContainer />
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Sold Listings</h2>

      {loading && <p className="text-center text-lg text-gray-600">Loading sold listings...</p>}
      {error && <p className="text-center text-lg text-red-600">{error}</p>}
      {!loading && listings.length === 0 && (
        <p className="text-center text-gray-600 text-lg mt-10">
          No sold listings available.
        </p>
      )}
      ...
      {selectedProperty && (
        <div className="modal-overlay fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="modal-content bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative p-6">
            ...
            <EditFormRenderer property={selectedProperty} onClose={closeModal} onUpdate={fetchSoldListings} />
          </div>
        </div>
      )}
