...
const ApartmentForm = ({ addProperty = () => {} }) => {
  ...
  const [numberOfBedrooms, setNumberOfBedrooms] = useState("Bedsitter");
  ...
  const [showModal, setShowModal] = useState(false); // Unused in provided snippet, but kept
  const [modalMessage, setModalMessage] = useState(""); // Unused
  const [modalType, setModalType] = useState(""); // Unused
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(null);
      addProperty(data);
      window.alert("Apartment added successfully!");
      // Clear form fields
      setPropertyName("");
      setDescription("");
      setLocation("");
      ...
      setError("Error adding apartment property. Please try again.");
      setSuccess(null);
      window.alert("Error adding apartment property. Please try again.");
    } finally {
    ...
    <>
      ...
      >
        ...
        <input
          ...
          placeholder="Upload the URL of your Apartment's location"
          className="px-2 py-1 md:px-4 md:py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-var(--secondary-color) bg-white w-full"
        />
