...
import { useLogin } from "../../context/IsLoggedIn"; // Add this import
import {
  Box,
  Typography,
  TextField,
...
import PaymentPollingModal from "../../components/Specific/DetailsDisplay/Bookings/createBooking/PaymentPollingModal";
...
export default function CheckoutPage() {
  ...
  // Get userData from context instead of localStorage
      ...
      if (paymentMethod === "mpesa") {
        ...
        if (paymentResult.success) {
          ...
          setPaymentData({
            ...
            phoneNumber: phone,
            tierId: selectedTier.id, // Add tier ID for subscription creation
          });
  ...
  const handlePaymentModalSuccess = async () => {
    try {
      // Create subscription after successful M-Pesa payment
  ...
  // Subscription creation function using context userData
  const createSubscription = async (tierId, paymentMethod = "mpesa", amountPaid = 0) => {
    try {
      // Calculate subscription dates
      const startDate = new Date().toISOString();
      const endDate = new Date();
      endDate.setMonth(endDate.getMonth() + 1); // Add 1 month for subscription
      
      // Get user_id from context instead of localStorage
...
// M-Pesa payment initiation function
...
// Card payment processing function
