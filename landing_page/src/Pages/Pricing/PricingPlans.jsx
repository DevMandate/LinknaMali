...
export default function PricingPlans() {
  ...
  // Handle login check for payment
  function handleSelectAndPay(tier) {
    if (isLoggedIn && userData) {
      // User is logged in, navigate to checkout page
      navigate(`/pricing/checkout/${tier.id}`);
    } else {
      // User is not logged in, show login prompt
      setPendingAction(true);
      const pendingPath = `/pricing/checkout/${tier.id}`;
      sessionStorage.setItem("pendingPath", pendingPath);
      setMessage(
        `Log in to proceed with your ${tier.name} plan purchase. If you don't have an account, Sign Up to get started.`
      );
      setAlertOpen(true);
    }
  }

  // Handle alert close (redirect to login)
  useEffect(() => {
    if (alertClose) {
      navigate(`/login`);
      setAlertClose(false);
    }
  }, [alertClose, navigate, setAlertClose]);

  // Check for pending pricing action after login
  useEffect(() => {
    if (isLoggedIn && userData) {
      const pendingPricing = sessionStorage.getItem("pendingPricing");
      if (pendingPricing) {
        try {
          const { action, tier } = JSON.parse(pendingPricing);
          if (action === "checkout") {
            setCheckoutTier(tier);
          }
          sessionStorage.removeItem("pendingPricing");
        } catch (error) {
          console.error("Error parsing pending pricing:", error);
        }
      }
    }
  }, [isLoggedIn, userData]);
  ...
  // Updated popularity assignment logic
  const { mostPopularId, popularId } = useMemo(() => {
    if (!tiers.length) return {};
    
    // Find specific tiers by name
    const growthTier = tiers.find(t => t.name === 'Growth');
    const maxTier = tiers.find(t => t.name === 'Max');
    
    return { 
      mostPopularId: maxTier?.id,     // Max gets "Most Popular"
      popularId: growthTier?.id       // Growth gets "Popular"
    };
  }, [tiers]);
